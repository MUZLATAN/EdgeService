 ...
 * ******************************************************************************
 * 文 件 名: engine_rknn.cpp
 * 创建日期: 2022-03-01
 * 作    者: rongzhaoyang (rongzhaoyang@whale.im)
 * 功能描述: RKNN推理引擎
 * ******************************************************************************
 */
#if defined (__PLATFORM_RV__)
...
IE_ERR_CODE EngineRKNN::get_input_attr()
{
    ...
    // 获取输入Tensor shape
    m_input_shape = IE_Shape(input_attr.dims[1], input_attr.dims[2], input_attr.dims[3]);    
...
IE_ERR_CODE EngineRKNN::get_output_attr()
{
    ...
    for (int i = 0; i < m_output_num; i++)
    {
        ...
        m_output_blobs.emplace_back(output_attr.name, output_attr.dims[1], output_attr.dims[2], output_attr.dims[3]);
        m_output_shapes.emplace_back(output_attr.dims[1], output_attr.dims[2], output_attr.dims[3]);
    }
    return IE_SUCCESS;
}
